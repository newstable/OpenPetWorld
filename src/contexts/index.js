...
import {PetSwapRouterContract, PetSwapFactoryContract, PetWorldCoin, WBNBAddress, PetSwapPair, StakingContract1, StakingContract2, ArtContract, EggContract, PetContract,MultiContract,MarketPlaceContract} from "../contract"
...
const DragonNameList = [
  "Akikiola","Asdrola","Bhala","Banduk","Dert","Dudha","Kullian","Ludianha","Eqopala","Eqopala",
  "Aiden", "Archion","Arrow","Brenton","Bullet","Daenerys","Dart","Deadheart","Devlin","Drache",
  "Draga", "Drago","Dragonite","Drake","Druk ","Egan ","Flurry ","Ghidorah ","Godzilla ","Helios ",
  "Knucker", "Maleficent","Mushu","Neptune","Ness","Nocturne","Oboth","Pendragon","Shenron","Tatsuya",
  "Typhon", "Anguis","Arad","Cadmus","Doryu","Draak","Draco","Draken","Drayce","Fafner",
  "Hydra", "Irad","Kaida","Kaliyah","Ladon","Libelle","Long","Melly","Meraxes","Ryoko",
  "Scylla", "Campe","Chimera","Druk"
]
...
switch (type) {
    ...
    case "UPDATE_MYARTTOKENS": {
        const { MyArtTokens } = payload
        return {
            ...state,
            MyArtTokens: MyArtTokens,
        }
        }
    case "UPDATE_MYEGGTOKENS": {
    const { MyEggTokens } = payload
    return {
        ...state,
        MyEggTokens: MyEggTokens,
    }
    }
    case "UPDATE_MYPETTOKENS": {
    const { MyPetTokens } = payload
    return {
        ...state,
        MyPetTokens: MyPetTokens,
    }
    }
    case "UPDATE_MARKETPLACE_PETTOKENS": {
      const { MARKETPetTokens } = payload
      return {
          ...state,
          MARKETPetTokens: MARKETPetTokens,
      }
    }
    case "UPDATE_BNBPRICE": {
    const { BNBPrice } = payload
    return {
        ...state,
        BNBPrice: BNBPrice,
    }
    }
    case "UPDATE_COINPRICE": {
    const { CoinPrice } = payload
    return {
        ...state,
        CoinPrice: CoinPrice,
    }
...
export default function Provider({ children }) {
    ...
    const updateMyArtTokens = useCallback((MyArtTokens) => {
        dispatch({
          type: "UPDATE_MYARTTOKENS",
          payload: {
            MyArtTokens,
          },
        })
    ...
    const updateMyEggTokens = useCallback((MyEggTokens) => {
        dispatch({
          type: "UPDATE_MYEGGTOKENS",
          payload: {
            MyEggTokens,
          },
        })
    ...
    const updateMyPetTokens = useCallback((MyPetTokens) => {
        dispatch({
          type: "UPDATE_MYPETTOKENS",
          payload: {
            MyPetTokens,
          },
        })
    ...
    const updateMARKETPLACEPetTokens = useCallback((MARKETPetTokens) =>{
      dispatch({
        type: "UPDATE_MARKETPLACE_PETTOKENS",
        payload: {
          MARKETPetTokens,
        },
      })
    ...
    const updateBNBPrice = useCallback((BNBPrice) => {
        dispatch({
          type: "UPDATE_BNBPRICE",
          payload: {
            BNBPrice,
          },
        })
    ...
    const updateCoinPrice = useCallback((CoinPrice) => {
        dispatch({
            type: "UPDATE_COINPRICE",
            payload: {
                CoinPrice,
            },
        })
    ...
    const petTokenUpdate = async () => {
        ...
        if(ids!==[]){
            var _tokenInfos = await MultiContract.getPetInfos(ids).catch((err) => {console.log(ids,err)});
      ...
      //egg token structure;
        ...
        PetTokens.owners.map((owner,index) => {
          if(owner.toUpperCase()===myAddress.toUpperCase()){
            ...
            var genepos = PetTokens._fGenome[index][4]+PetTokens._fGenome[index][5]*4+PetTokens._fGenome[index][6]*16;
            names.push(DragonNameList[genepos]);
          }
        })
        updateMyPetTokens({creators,owners,_fGenome,_fID,_gene,_mGenome,_mID,tokenIds,img,names});
    }
    ...
    //my tokenUpdate
    useMemo(()=>{
      if(wallet.status==="connected" && wallet.account!==null && wallet.account!==undefined){
    ...
    useEffect(() => {
      var images = [];
      for(var i=0; i<12; i++){
          images.push({id:1,img:dark1,title:"Earth Dragon",price:80,gene:"AAA*"});
          images.push({id:2,img:fire1,title:"Fire Dragon",price:80,gene:"CCC*"});
          images.push({id:3,img:frozen1,title:"Ice Dragon",price:80,gene:"GGG*"});
          images.push({id:4,img:water1,title:"Water Dragon",price:80,gene:"TTT*"});
          images.push({id:5,img:dark2,title:"Earth Dragon",price:40,gene:"AA**"});
          images.push({id:6,img:fire2,title:"Fire Dragon",price:40,gene:"CC**"});
          images.push({id:7,img:frozen2,title:"Ice Dragon",price:40,gene:"GG**"});
          images.push({id:8,img:water2,title:"Water Dragon",price:40,gene:"TT**"});
          images.push({id:9,img:dark3,title:"Earth Dragon",price:20,gene:"****"});
          images.push({id:10,img:fire3,title:"Fire Dragon",price:20,gene:"****"});
          images.push({id:11,img:frozen3,title:"Ice Dragon",price:20,gene:"****"});
          images.push({id:12,img:water3,title:"Water Dragon",price:20,gene:"****"});
      };
      tokenUpdates();
    },[]);
